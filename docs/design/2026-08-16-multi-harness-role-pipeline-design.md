...
| 42 | **Operator override, 2026-08-19: decision 26's sequencing is set aside for one build.** All four plans (rust-runner, deliver-preflight, dashboard, planning-skill) are implemented together, offloaded to `pi`/`deepseek-v4-pro` in an isolated worktree, with no second-harness cost measurement preceding the build | Explicit operator instruction, made after decision 26's condition was restated and the contradiction named. The risk decision 26 was written to avoid — building the tier ladder, adapters, preflight and dashboard from a sample of one unmeasured harness — is accepted knowingly, not by drift. Does not reopen decision 26 for the runner's own future adapter work; it authorises this one build |
